...
const BirthChartVisualizer: React.FC<BirthChartVisualizerProps> = ({ result, t, darkMode }) => {
  ...
  const aspectCounts = React.useMemo(() => {
    if (!result) return {};
    return result.aspects.reduce((acc, aspect) => {
  ...
  }, [result, darkMode, t, signs, planetData]);
      ...
      <div className="flex flex-col items-center justify-center p-6 rounded-3xl glass-card border border-white/5 relative overflow-hidden">
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-4">
          {planetData.map(p => (
            <div 
              key={p.name} 
              className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/5 border border-white/5 cursor-help hover:scale-105 hover:bg-white/10 transition-all"
