...
const BMICalculator: React.FC = () => {
  ...
  const calculateBMI = () => {
    ...
    if (w > 0 && h > 0) {
      const bmi = w / (h * h);
      let category = '';
      if (bmi < 18.5) category = lang === 'en' ? 'Underweight' : 'Bajo peso';
      else if (bmi < 25) category = lang === 'en' ? 'Normal weight' : 'Peso normal';
      else if (bmi < 30) category = lang === 'en' ? 'Overweight' : 'Sobrepeso';
      else category = lang === 'en' ? 'Obesity' : 'Obesidad';
  ...
  return (
    ...
    >
      ...
      {result && (
        ...
        >
          <div className="text-sm font-bold uppercase tracking-widest text-blue-500 mb-2">Result</div>
          <div className="text-5xl font-black mb-2">{result.bmi}</div>
          <div className="text-xl font-bold opacity-80">{result.category}</div>
        </motion.div>
      )}
    </motion.div>
  );
